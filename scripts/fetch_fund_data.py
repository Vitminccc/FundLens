...
# ---------- 4. 指数K线（市场模式判断） ----------
def fetch_index_kline(code='000300', days=120):
    """抓取指数近 days 个交易日的日K线收盘价，返回 [(date_str, close), ...]。
    用于判断市场模式（牛市/熊市/均衡）。"""
    secid = f"1.{code}"
...
# ---------- 5. 四机构评级（上海证券/招商证券/济安金信/晨星） ----------
def fetch_ratings(code):
    """从天天基金评级API抓取4机构评级，返回 {date, 招商证券, 上海证券3年, 上海证券5年, 济安金信, 晨星, avg_star, star_count}。
    星级为 1-5 的 float 或 None（无数据）。全空返回 None。"""
    url = f"https://api.fund.eastmoney.com/F10/JJPJ/?fundcode={code}&pageIndex=1&pageSize=10"
    txt = get(url, referer="https://fundf10.eastmoney.com/")
    if not txt:
        return None
    try:
    ...
    for row in rows:
        stars = []
        mapping = {
            "招商证券": row.get("ZSPJ"),       # 招商证券评级
            "上海证券3年": row.get("SZPJ3"),     # 上海证券3年期
            "上海证券5年": row.get("ZSPJ5"),     # 上海证券5年期
            "济安金信": row.get("JAPJ"),         # 济安金信评级
            "晨星": row.get("CXPJ3"),           # 晨星3年期
        }
        result = {"date": row.get("RDATE", "")}
        valid_stars = []
        five_star_count = 0
        for name, val in mapping.items():
            if val is not None and str(val).strip() and str(val).strip() != "0":
                ...
                result[name] = None
        if valid_stars:
            result["avg_star"] = round(sum(valid_stars) / len(valid_stars), 2)
            result["star_count"] = len(valid_stars)
            result["five_star_count"] = five_star_count
            return result
    return None
