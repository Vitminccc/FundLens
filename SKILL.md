...
description: >-
  ...
  (7) 双层评分——给一只基金同时输出郑希风格契合度（6维AI打分）+ 综合质量评分（7维量化自动计算）+ 市场模式 + 4种买点信号 + 行业归一化。
  ...
  a commentary 用郑希口吻, to check words vs holdings, or to look up/compare/score ANY China mutual fund (任意基金/某基金经理/同类对比/给基金打分/综合评分/买点信号/市场模式)——use this skill, even if they
...
用户说"给 XX 基金打个分""用郑希的标准评评这只基金""这只基金郑希会买吗""这只基金质量怎么样""有没有买点信号"时：

1. **一条命令搞定准备**：`python scripts/score_fund.py <代码或名称>`。它会自动：解析代码 → 备齐数据（郑希的用精编快照，别的实时抓取并缓存）→ **自动计算综合质量评分（7维量化）** → 输出市场模式 + 买点信号 → 打印郑希风格契合度证据档案。**不用再单独跑 lookup/fetch/读文件**。
2. **按评分卡打分**：读 `references/scorecard.md`，它分两层：
   - **Part B（综合质量评分）**：7 维纯量化，脚本已自动算好（收益能力/风险控制/收益质量/基金评级/基金经理/持仓质量/赚钱效应），含市场模式加权 + 行业归一化 + 买点信号——直接引用脚本输出即可。
   - **Part A（郑希风格契合度）**：6 维（景气方向·ROE低位弹性·全球比较优势·流动性·集中度与周期拼接·业绩印证），需你逐项给分，给总分、评级、理由，并和郑希自己的基金对比风格。
3. **合成双层报告**：按 `references/scorecard.md` 的输出模板，把两层评分 + 买点信号 + 桥接说明合在一起。当风格契合度与综合质量差异大时（如低契合 + 高质量），用桥接说明解释。
4. **守住定位**：Part A 衡量"**与郑希投资风格的契合度**"，不是基金好坏的绝对评判——防御型/红利/纯债基金天然低分很正常，要讲清楚，别让用户误以为"低分=差基金"。Part B 衡量"**基金综合质量**"，也不是投资建议。个股 ROE、流动性等无数据项标"需核实"。
...
- `scripts/score_fund.py <代码或名称>` —— **双层评分一键入口**：自动找代码+备数据+算全部指标，输出综合质量评分（7维量化）+ 市场模式 + 买点信号 + 郑希风格契合度证据档案，配合 `references/scorecard.md` 打分。可选分析周期参数（7d/30d/90d/180d/365d）。
- `references/scorecard.md` —— 双层评分卡：Part A 郑希风格契合度（六维规则、打分区间）+ Part B 综合质量评分（七维计算规则、市场模式权重切换、买点信号、行业归一化、双层交互规则、输出模板）。
